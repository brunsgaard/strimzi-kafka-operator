...
public class KafkaUserOperator {
    ...
    private CompletionStage<Void> reconcileCredentialsQuotasAndAcls(Reconciliation reconciliation, KafkaUserModel user, Secret userSecret, KafkaUserStatus userStatus)   {
        ...
        if (config.isKraftEnabled()) {
            ...
            scramCredentialsFuture = scramCredentialsOperator.reconcile(reconciliation, user.getName() + user.getPrincipalSuffix(), user.getScramSha512Password());
        }
        ...
        CompletionStage<ReconcileResult<KafkaUserQuotas>> tlsQuotasFuture = quotasOperator.reconcile(reconciliation, KafkaUserModel.getTlsUserName(reconciliation.name()) + user.getPrincipalSuffix(), tlsQuotas);
        CompletionStage<ReconcileResult<KafkaUserQuotas>> quotasFuture = quotasOperator.reconcile(reconciliation, KafkaUserModel.getScramUserName(reconciliation.name()) + user.getPrincipalSuffix(), scramOrNoneQuotas);
        ...
        if (config.isAclsAdminApiSupported()) {
            aclsTlsUserFuture = aclOperator.reconcile(reconciliation, KafkaUserModel.getTlsUserName(reconciliation.name()) + user.getPrincipalSuffix(), tlsAcls);
            aclsScramUserFuture = aclOperator.reconcile(reconciliation, KafkaUserModel.getScramUserName(reconciliation.name()) + user.getPrincipalSuffix(), scramOrNoneAcls);
        } else {
